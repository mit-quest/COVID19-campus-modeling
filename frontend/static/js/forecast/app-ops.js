...
function generateHome() {
  ...
  Promise.all([
    ...
    uuids.forEach(uuid => {
        ...
        willWorkQ = Number(uuid) > 1602306000;
        if (willWorkQ) {
            element.innerHTML = '<a href="../forecast?page=simulation&uuid=' + uuid + '" target="_blank">' + uuid + ' </a>';
        } else {
...
function generateReport() {
  ...
  Promise.all([
    getData()
      ...
      // metadata
      if (loggedIn) {
        t0Date = getNested(allData, 'scenario', 'metadata', 'scenario_parameters', 't0_date');
        nDays = getNested(allData, 'scenario', 'metadata', 'scenario_parameters', 'n_days_to_simulate');
        if ( t0Date != undefined && nDays != undefined ) {
          displayt0(t0Date, nDays);
        }
      }
      ...
      if (loggedIn) {
        ...
        addSectionBreak(questionID, 'Demographics', '');

        data = getNested(allData, 'analysis', 'demographic-analysis-statistics', 'results', 'simulation',
          'total_population_size', 'full_simulation_time_window', 'all_buildings', t0Date + ' 00:00:00', 'count');
        if ( data != undefined ) {
          plotName = 'total-population-size';
          plotData[questionID + '-' + plotName] = data;
          initializeDataViz('col-12', questionID, plotName, [], []);
        }

        data = getNested(allData, 'analysis', 'demographic-analysis-statistics', 'results', 'simulation', 'age_bin', 'full_simulation_time_window', 'all_buildings');
        if ( data != undefined ) {
          plotName = 'age-bin-chart';
          plotData[questionID + '-' + plotName] = Object.values(data)[0];
          initializeDataViz('col-6', questionID, plotName, [], []);
        }

        data = getNested(allData, 'analysis', 'demographic-analysis-statistics', 'results', 'simulation', 'mit_person_type', 'full_simulation_time_window', 'all_buildings');
        if ( data != undefined ) {
          plotName = 'job-category-chart';
          plotData[questionID + '-' + plotName] = Object.values(data)[0];
          initializeDataViz('col-6', questionID, plotName, [], []);
        }

        data = getNested(allData, 'analysis', 'demographic-analysis-statistics', 'results', 'simulation', 'commuting_type', 'full_simulation_time_window', 'all_buildings');
        if ( data != undefined ) {
          plotName = 'commute-type-chart';
          plotData[questionID + '-' + plotName] = Object.values(data)[0];
          initializeDataViz('col-6', questionID, plotName, [], []);
        }

        data = getNested(allData, 'analysis', 'demographic-analysis-statistics', 'results', 'simulation', 'home_zip', 'full_simulation_time_window', 'all_buildings');
        if ( data != undefined ) {
          plotName = 'home-zipcode-chart';
          plotData[questionID + '-' + plotName] = Object.values(data)[0];
          initializeDataViz('col-6', questionID, plotName, [], []);
        }
      }
      ...
      // simulation-2: facilities and mobility
      if (loggedIn) {
        questionID = 'simulation-2';

        addSectionBreak(questionID, 'Campus-wide Stats', '');

        if (
          'analysis' in allData &&
          'campus-statistics' in allData['analysis'] &&
          'results' in allData['analysis']['campus-statistics'] &&
          'simulation' in allData['analysis']['campus-statistics']['results']
        ) {

          data = getNested(allData, 'analysis', 'campus-statistics', 'results', 'simulation', 'distinct_individual_visits');
          plotName = 'campus-visits-time-plot';
          plotData[questionID + '-' + plotName] = data;
          initializeDataViz('col-6', questionID, plotName, [], []);

          data = getNested(allData, 'analysis', 'campus-statistics', 'results', 'simulation', 'inflow');
          plotName = 'campus-inflow-time-plot';
          plotData[questionID + '-' + plotName] = data;
          initializeDataViz('col-6', questionID, plotName, [], []);

          data = getNested(allData, 'analysis', 'campus-statistics', 'results', 'simulation', 'outflow');
          plotName = 'campus-outflow-time-plot';
          plotData[questionID + '-' + plotName] = data;
          initializeDataViz('col-6', questionID, plotName, [], []);
      ...
      if (loggedIn) {
        ...
        if (
          'analysis' in allData &&
          'building-occupancy-statistics' in allData['analysis'] &&
          'results' in allData['analysis']['building-occupancy-statistics'] &&
          'simulation' in allData['analysis']['building-occupancy-statistics']['results']
        ) {

          linkedPlotNames = [
            'building-visits-time-plot',
            'building-density-time-plot',
          ...
          linkedControls = [
            ...
            'control-' + questionID + '-' + 'building-density-time-plot',
            'control-' + questionID + '-' + 'building-inflow-time-plot',
            'control-' + questionID + '-' + 'building-outflow-time-plot',
          ];

          data = getNested(allData, 'analysis', 'building-occupancy-statistics', 'results', 'simulation', 'distinct_individual_visits');
          plotName = 'building-visits-time-plot';
          plotData[questionID + '-' + plotName] = data;
          initializeDataViz('col-6', questionID, plotName, linkedPlotNames, linkedControls);

          data = getNested(allData, 'analysis', 'building-occupancy-statistics', 'results', 'simulation', 'density');
          plotName = 'building-density-time-plot';
          plotData[questionID + '-' + plotName] = data;
          initializeDataViz('col-6', questionID, plotName, linkedPlotNames, linkedControls);

          data = getNested(allData, 'analysis', 'building-occupancy-statistics', 'results', 'simulation', 'inflow');
          plotName = 'building-inflow-time-plot';
          plotData[questionID + '-' + plotName] = data;
          initializeDataViz('col-6', questionID, plotName, linkedPlotNames, linkedControls);

          data = getNested(allData, 'analysis', 'building-occupancy-statistics', 'results', 'simulation', 'outflow');
...
async function getUUIDs() {
  ...
  var url = getUrl.protocol + "//" + getUrl.host + "/get-uuids/forecast";
  console.log({"url": url});
...
async function getData() {
  ...
  var getUrl = window.location;
  var url = getUrl.protocol + "//" + getUrl.host + "/get-data/forecast";
  console.log({"url": url});
