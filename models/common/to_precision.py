__author__ = '''
code from https://bitbucket.org/william_rusnack/to-precision/src/master/to_precision.py
...
def _std_notation(value, precision, _, strip_zeros, preserve_integer):
    ...
    result = ('-' if is_neg else '') + \
        _place_dot(sig_digits, power, strip_zeros)
...
def sig_fig_formatted(numbah, int_only=False, num_digits=2):
    if not isnan(numbah):
        # return float(round(numbah, -int(floor(log10(abs(numbah)))) + num_digits - 1))
        if int_only == False:
            return float(to_precision(numbah, num_digits))
        else:  # if number returned has to be an int (e.g. for demographic stats)
            return int(numbah)
